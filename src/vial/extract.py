...
=============================================================
Extrae la red vial real desde OpenStreetMap para el visualizador (overlay
encima del mapa de zonificación).

Salida (`visualizer/cities/<slug>/datos_vial.json`, formato compacto de
shared/compact.py): una capa por categoría — highway, major, minor, local,
pedestrian, bike — con items { id, name, coords: [[lat,lon],...], cs2_key, bridge }.
El visualizer la decodifica a `DATA_VIAL = { categoría: [items] }`.
...
def main():
    ...
    out_path = out_dir / compact_filename("vial")
    ...
    write_compact(out_path, "vial", dict(buckets), meta=meta)
    remove_legacy(out_dir, "vial")
