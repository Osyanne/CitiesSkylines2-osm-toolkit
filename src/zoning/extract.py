...
===============================================================
Extrae polígonos de zonificación real desde OpenStreetMap y los exporta a
`visualizer/cities/<slug>/datos_zonificacion.json` (formato compacto, ver
shared/compact.py), una capa por cs2_key.
...
def main():
    ...
    out_path = out_dir / compact_filename("zoning")
    ...
    write_compact(
        out_path,
        "zoning",
        {key: output[key] for key in CS2_LABELS},
        meta={
            "generator": "extract_zoning.py",
            "slug": slug,
            "bbox": bbox,
            "generated_at": ts,
            "model": "CS2-aligned: 13 zones (6 res + 2 com + 2 office + 1 ind + 2 prk)",
            "total_features": total,
        },
    )
    remove_legacy(out_dir, "zoning")
