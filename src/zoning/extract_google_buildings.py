...
Pipeline:
  ...
  6. Output: visualizer/cities/<slug>/datos_external_buildings.json (formato
     compacto, una capa por zona) — el visualizer lo decodifica a arrays
     `DATA_EXT_<ZONE>` paralelos a los DATA_<ZONE> de zoning.
...
def main():
    ...
    out_path = out_dir / compact_filename("external_buildings")
    ...
    write_compact(
        out_path,
        "external_buildings",
        {key: output.get(key, []) for key in CS2_LABELS},
        meta={
            "generator": "extract_google_buildings.py",
            "slug": args.city,
            "source": "Google Open Buildings v3 augmentation",
            "bbox": bbox_str,
            "generated_at": ts,
    ...
    remove_legacy(out_dir, "external_buildings")
